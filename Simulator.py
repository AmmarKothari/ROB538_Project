...
class Simulator(object):
	...
	def sim_step(self, pop_set):
		...
		for poi in self.poi_list:
			...
			poi.pos_history.append(poi.pos)

		# Rovers step
		for rover in self.rover_list:
			inputs = self.return_NN_inputs(rover)
			rover.population[pop_set].performance += sum(inputs[4:8])
			# if rover.population[pop_set].performance < sum(inputs[4:8]):
				# rover.population[pop_set].performance = sum(inputs[4:8])
			outputs = rover.population[pop_set].forward(inputs)
			rover.sim_step(outputs, self.world_width, self.world_height)
			rover.pos_history.append(poi.pos)
	...
	# 	epsilon_choice = np.random.rand(int(k))
	# 	for i in range(int(k)):
	# 		worst = min(nn_list, key=attrgetter('performance'))
	# 		# # random.shuffle(nn_list)
	# 		# if epsilon_choice[i] > epsilon:
	# 		# 	worst = min(nn_list, key=attrgetter('performance'))
	# 		# else:
	# 		# 	rand_choice = random.randint(0,int(len(nn_list)/2))
	# 		# 	worst = nn_list[rand_choice]
	...
	def return_POI_vel(self, poiList, rover, max_dist = 500):
                ...
                for poi in poiList:
                    ...
                    rel_vel_vect =  poi_vel_vect - rover_vel_vect
	...
	def return_NN_inputs(self, rover):
		...
		for i in range(4):
			inputs.append(vel_sensor_vals[i][0])
			inputs.append(vel_sensor_vals[i][1])
		return inputs
# =======================================================
# =======================================================
